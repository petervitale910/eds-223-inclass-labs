...
#| message: false
#| warning: false
pacman::p_load('geodata', 'tidyverse', 'terra', 'tmap', 'kableExtra',
               'spData', 'spDataLarge')
...
# Find the number of layers of raster
nlyr(landsat)

# Find names of layers
names(landsat)
```
...
```{r}
# create a point withing area covered by raster
point <- matrix(c(-113,37.5), ncol = 2)

# approach 1: find Cell ID
id <- cellFromXY(zion_elevation, xy = point)

# index to cell
zion_elevation[id]

## Appeoach 2: terra extract
# extract raster value at point 
...
rcl <- matrix(c(1000, 1500, 1, # Group 1
                1500, 2000, 2, # Group 2
                2000, 2500, 3, # Group 3
                2500, 3000, 4),# Group 4
              ncol = 3, byrow = TRUE)

# Use Reclass matrix to reclassify elevation raster

reclassified <-  classify(zion_elevation, rcl = rcl)

# Change reclassified to factors
...
ndvirast <- lapp(landsat[[c(4,3)]], fun = ndvi_fun)
