...
```{r}
pacman::p_load("tmap", 'stars', 'sf', 'here')
```
...
```{r}
# plotting a single spatial opject 
...
```{r}
tm_shape(world_vector)+ #country data
  tm_polygons()+
tm_shape(world_cities)+ #Defines second thing
  tm_dots() #How to plot second object
...
tm_shape(world_cities)+ #Defines second thing
  tm_dots()+
  tm_text('name', size = .7, col = 'blue', zindex = 3)+
tm_shape(world_elevation)+
  tm_raster(zindex = 1)+ #Working with raster data
tm_shape(world_vector)+ #country data
  tm_borders(zindex = 2) #Zindex specifies what on top of what
...
```{r}
#Oh lmao this does the same thing as zindex from the last chunk 
tm_shape(world_elevation) +
  tm_raster()+
tm_shape(world_vector)+
  tm_borders()+
tm_shape(world_cities, is.main = TRUE)+
  tm_dots()+
  tm_text('name')
```
...
```{r}
tm_shape(world_elevation, bbox = c(5, 36, 20, 48))+ #add bounding box to restrict extent
  tm_raster()+
tm_shape(world_vector)+
  tm_borders()+
tm_shape(world_cities)+
  tm_dots()+
  tm_text('name', zindex = 0)
```
...
```{r}
tm_shape(world_elevation, bbox= world_cities)+ #bounding boc = extent of world cities+
  tm_raster()+
tm_shape(world_vector)+
  tm_borders()
...
```{r}
tm_shape(world_elevation, bbox = 'Hawaii')+
  tm_raster()+
tm_shape(world_vector)+
  tm_borders()
...
```{r}
#fill polygon with a color palette representing variab;e
tm_shape(world_vector)+
...
tm_shape(world_vector)+
  tm_polygons(fill = 'life_expectancy',
              fill.scale = tm_scale(values = c('blue', 'goldenrod')),
...
tm_shape(world_vector)+
  tm_polygons(fill = 'life_expectancy',
              fill.scale = tm_scale(values = colorspace::terrain_hcl(6)))
...
tm_shape(world_vector)+
  tm_polygons(fill = 'wb_income_region',
              fill.legend = tm_legend(title = "Region Income"),
...
tm_shape(world_vector) +
  tm_polygons(fill = 'gdp_per_cap',
              fill.lwfwnd = tm_legend(title = 'GDP Per Capita'))
```
