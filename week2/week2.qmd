...
```{r}
pacman::p_load('sf', 'tmap', 'tidyverse', 'spdata')
```
...
```{r}
london_point <- st_point(c(.1,51.5))
# Add crs
...
if (st_crs(dataset1) != st_crs(dataset2)){
  print('warning! headed abort the pregnancy')
...
world %>% 
  ...
  slice_max(lifeExp) %>% #Slices at the maximum
  st_drop_geometry()
```
...
pop_density <- world %>% 
  group_by(continent) %>% 
  summarize(density = sum(pop, na.rm = TRUE)/ sum(area_km2, na.rm = TRUE)) %>% 
...
if (nrow(world_coffee_inner) != nrow(coffee_data)){
  warning( 'inner join does not match original data')
}
