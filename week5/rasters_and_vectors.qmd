...
# Convert high elevation pixels into polygon shapes and then transform into a simple feature
...
pacman::p_load('rayshader', 'magick', 'osmdata', 'raster')

```

```{r}

bryce = raster("week5/data/Bryce_Canyon.tif")
...
```{r}
# Create extent object from bounding box coords
...
# Convert the cropped raster to matrix for rayshader
...
# Querying OSM for all 'highway' features
...
```{r}
# Create combined trails layer with different styles for each typen
