...
# Filter to region of Canterbury
canterbury <- nz %>% 
...
```{r}
# subset nz_height to just the features that intersect Canterbury
c_height_1 = nz_height[canterbury, ]
```
...
#Check that intersect is oposite of disjoint

nrow(c_height_1) + nrow(c_height_disjoint) == nrow(nz_height)
```
...
c_height_2 <-  nz_height %>% 
  st_filter(y = canterbury, .predicate = st_intersects) # x = nz_height, y within x 
```
...
nz_height_sgbp <- st_intersects(x = nz_height, y = canterbury);nz_height_sgbp

# Convert to logical vector
nz_height_logical <- lengths(nz_height_sgbp) > 0; nz_height_logical #Logical vector returns bool
...
```{r}
# Find heights within 1000 km from Canterbury
...
tm_shape(nz)+
  tm_polygons()+
tm_shape(canterbury)+
  tm_fill(fill = 'goldenrod')+
tm_shape(c_height4)+
  tm_dots(fill = 'forestgreen')
```
...
```{r}
#| eval: false
joined_df <- left_join(df_A, df_B, by = c('id_A' = 'id_B'))

 st_join (x,y, join = st_within) #Using geometry
...
nz_height_left_join <- st_join(nz_height, nz) %>% 
  select(id = t50_fid, elevation, region = Name); head(nz_height_left_join)
...
tmap_mode('view')
tm_shape(cycle_hire)+
  tm_symbols(fill = 'red', fill_alpha = .2)+
tm_shape(cycle_hire_osm)+
  tm_symbols(fill = 'blue', fill_alpha = .2)
```
...
cycle_hire_join <- st_join(cycle_hire, cycle_hire_osm, 
                           st_is_within_distance, dist = units::set_units(20, 'm')) %>% 
  select(id, capacity)
...
#Use area-weighted interpolation to aggregate the 'value' attribute

aggregating_zones_area_weighted <- st_interpolate_aw(x = incongruent_2,
                                                     aggregating_zones,
                                                     extensive = TRUE) #Set true for counts or population, false on density or weight 
...
#Same thing with aggregate function
regions2 <- aggregate(x = us_states[, 'total_pop_15'],
...
#combine texas and western states 
...
tmap_arrange(map1, map2, nrow = 1)
```
You can see that the centroid and pos are generally overlapping
### Simplification
