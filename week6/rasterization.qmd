...
template_raster_low <- terra::rast(ext(park_polygon), resolution = .05, crs = st_crs(park_polygon))

# create high spatial resolution template raster
template_raster_high <-  terra::rast(ext(park_polygon), resolution = .01, crs = st_crs(park_polygon))

# Rasterize low resolution 
polygon_raster_low <-  terra::rasterize(park_polygon, template_raster_low)

# Rasterize high resolution 
...
map2 <- tm_shape(polygon_raster_high) +
  tm_raster() +
  tm_title(text = "high resolution 6/7") + 
    tm_layout(
    legend.position = c("left", "bottom")  
  )
...
```{r}
ch_raster2 <-  terra::rasterize(cycle_hire_osm_projected, template_raster, fun = 'length')
```
...
# apply NDVI function to Landsat bands 3 & 4
ndmi_rast <- lapp(L7[[c(4,2)]], fun = ndmi_fun)

names(ndmi_rast) <- 'NDMI'

# Visualize NDVI
ndmi_map <- tm_shape(ndmi_rast) +
...
tmap_arrange(map1, map2, ndmi_map)
```
